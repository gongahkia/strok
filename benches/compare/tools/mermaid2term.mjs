...
const defaultOutDir = join(repoRoot, "benches/compare/results/mermaid2term");
const defaultBin = join(repoRoot, "node_modules/.bin/mermaid2term");

function usage() {
  return `Usage:
  node benches/compare/tools/mermaid2term.mjs [--manifest FILE] [--out-dir DIR] [--allow-failures]
  node benches/compare/tools/mermaid2term.mjs --input FILE [--input FILE...] [--out-dir DIR]
  node benches/compare/tools/mermaid2term.mjs --input FILE --stdout

Options:
  --bin FILE         mermaid2term binary path. Defaults to MERMAID2TERM_BIN or local node_modules.
...
function parseArgs(args) {
  const options = {
    bin: process.env.MERMAID2TERM_BIN || defaultBin,
    inputs: [],
