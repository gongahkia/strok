...
const defaultOutDir = join(repoRoot, "benches/compare/results/alexander-mermaid-ascii");

function usage() {
  return `Usage:
  node benches/compare/tools/alexander-mermaid-ascii.mjs [--manifest FILE] [--out-dir DIR] [--allow-failures]
  node benches/compare/tools/alexander-mermaid-ascii.mjs --input FILE [--input FILE...] [--out-dir DIR]
  node benches/compare/tools/alexander-mermaid-ascii.mjs --input FILE --stdout

Options:
  --bin FILE         mermaid-ascii binary path. Defaults to MERMAID_ASCII_BIN or PATH lookup.
...
function parseArgs(args) {
  const options = {
    bin: process.env.MERMAID_ASCII_BIN || "mermaid-ascii",
    inputs: [],
...
async function main() {
  ...
  if (!options.stdout) {
    process.stdout.write(`${JSON.stringify({
      tool: "AlexanderGrooff/mermaid-ascii",
      binary: options.bin,
      results,
    }, null, 2)}\n`);
  }
