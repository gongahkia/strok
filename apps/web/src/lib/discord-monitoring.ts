export type DiscordMetricName =
  | "discord_install_total"
  | "discord_metrics_request_total"
  | "discord_search_total";

const counters = new Map<string, { labels: Record<string, string>; value: number }>();

export function incrementDiscordMetric(
  name: DiscordMetricName,
...
export function discordPrometheusMetrics(): string {
  const lines: string[] = [];
...
export function resetDiscordMetricsForTest(): void {
  counters.clear();
}
