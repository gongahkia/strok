export type DiscordMetricName =
  | "discord_command_total"
  | "discord_http_request_total"
  | "discord_interaction_total"
  | "discord_lookup_total"
  | "discord_register_commands_total"
  | "discord_write_total";

export interface DiscordMonitor {
  increment(name: DiscordMetricName, labels?: Record<string, string | number | boolean>): void;
  log(event: string, fields?: Record<string, unknown>): void;
  prometheus(): string;
}

export class InMemoryDiscordMonitor implements DiscordMonitor {
...
let defaultMonitor: DiscordMonitor | undefined;

export function defaultDiscordMonitor(): DiscordMonitor {
  defaultMonitor ??= new InMemoryDiscordMonitor();
...
function metricKey(name: DiscordMetricName, labels: Record<string, string>): string {
