...
- API identity comes from a DB-backed team API key and may include a user ID header.
...
- MCP reads `WAT_API_BASE_URL` and `WAT_API_KEY` from its process environment and lets the web API resolve the DB-backed team key.
...
- MCP tools call the same web API paths as other integrations.
...
- `apps/web/src/lib/search-data.ts`: bridge from DB-backed repositories to search entries.
- `apps/web/src/lib/team-entries.ts`, `personal-entries.ts`, `suggestions.ts`, `audit-log.ts`: DB-backed repository helpers with test fixtures.
