...
def run_case(binary: Path, source: Source, case: Case, backend: str, repeats: int, warmups: int, work: Path, font: str | None, workers: int | None) -> dict[str, object]:
  ...
  if workers is not None:
    env["CONTOURTTY_WORKERS"] = str(workers)
    command_prefix = f"CONTOURTTY_WORKERS={workers} "
...
def markdown_table(rows: list[dict[str, object]], threshold: float, repeats: int, frames: int, workers: int | None) -> str:
  worker_note = f" with `CONTOURTTY_WORKERS={workers}`" if workers is not None else ""
  lines = [
...
def main() -> None:
  ...
  parser.add_argument("--binary", default="build/package/contourtty")
  parser.add_argument("--work", default="/tmp/contourtty-p8-final")
  parser.add_argument("--out", default="/tmp/contourtty-p8-final/P8_FINAL_BENCHMARKS.md")
