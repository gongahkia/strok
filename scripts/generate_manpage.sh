...
mkdir -p "$(dirname "$out")"
{
  ...
  printf 'A profile=live|structure|low-bandwidth|export entry selects a built-in profile.\n'
  printf 'Config profile, config entries, CLI profile, then CLI flags are applied in order.\n'
  printf '.SH EXAMPLES\n'
  printf '.nf\n'
