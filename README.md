...
What's new on the v1.0 track: live OSD tuning, A/B split view, still snapshots, temporal glyph stabilization, opt-in temporal supersampling for low-fps structure analysis, NPR styles, octant/sextant/braille blitters, graph YAML loading, new input paths, local package generation, macOS Metal Shadertoy-style shader input, and an optional Vulkan structure-analysis backend. Hosted Linux Vulkan proof, live Sixel terminal proof, and release bottles remain pending.
...
| `--ffmpeg-log off\|error\|warning\|info\|debug\|trace` | Route opt-in FFmpeg diagnostics into `--log FILE`. |
| `--metrics-jsonl FILE` | Write per-window terminal-playback metrics in JSON Lines format. |
...
RGB frame -> optional style pre-pass -> luminance -> contrast/DoG -> Sobel
          -> optional ETF/temporal supersample/optical-flow/temporal history
