...
#[test]
fn gitgraph_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/gitgraph/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/gitgraph")
        ...
        assert!(matches!(diagram.kind, DiagramKind::GitGraph(_)));
