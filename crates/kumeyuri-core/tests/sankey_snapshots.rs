...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_quoted_comments"];

#[test]
fn sankey_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/sankey/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/sankey")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Sankey(_)));
