...
const SNAPSHOT_NAMES: [&str; 3] = [
    "regional_indicators_flowchart",
    "skin_tone_modifiers_flowchart",
    "zwj_sequences_flowchart",
];

#[test]
fn emoji_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/emoji/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/emoji")
