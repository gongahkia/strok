...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_nested_shapes_links"];

#[test]
fn block_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/block/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/block")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Block(_)));
