...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_keyed_options"];

#[test]
fn radar_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/radar/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/radar")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Radar(_)));
...
#[test]
fn radar_animation_collapses_to_static_frame() {
    let source = include_str!("../../../tests/snapshots/radar/input/02_keyed_options.mmd");
