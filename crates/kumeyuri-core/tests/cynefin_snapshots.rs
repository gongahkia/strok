...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_transitions"];

#[test]
fn cynefin_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/cynefin/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/cynefin")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Cynefin(_)));
...
#[test]
fn cynefin_animation_collapses_to_static_frame() {
    let source = include_str!("../../../tests/snapshots/cynefin/input/02_transitions.mmd");
