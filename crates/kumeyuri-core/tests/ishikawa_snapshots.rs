...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_deep_comments_directive"];

#[test]
fn ishikawa_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/ishikawa/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/ishikawa")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Ishikawa(_)));
...
#[test]
fn ishikawa_animation_collapses_to_static_frame() {
    let source =
        include_str!("../../../tests/snapshots/ishikawa/input/02_deep_comments_directive.mmd");
