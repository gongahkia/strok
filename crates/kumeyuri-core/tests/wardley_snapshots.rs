...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_annotations_pipeline"];

#[test]
fn wardley_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/wardley/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/wardley")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Wardley(_)));
...
#[test]
fn wardley_animation_collapses_to_static_frame() {
    let source = include_str!("../../../tests/snapshots/wardley/input/02_annotations_pipeline.mmd");
