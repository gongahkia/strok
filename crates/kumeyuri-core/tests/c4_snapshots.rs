...
const SNAPSHOT_NAMES: [&str; 3] = ["01_context", "02_container_boundary", "03_deployment"];

#[test]
fn c4_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/c4/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/c4")
        ...
        assert!(matches!(diagram.kind, DiagramKind::C4(_)));
