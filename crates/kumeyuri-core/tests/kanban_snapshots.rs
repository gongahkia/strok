...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_metadata"];

#[test]
fn kanban_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/kanban/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/kanban")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Kanban(_)));
...
#[test]
fn kanban_animation_collapses_to_static_frame() {
    let source = include_str!("../../../tests/snapshots/kanban/input/02_metadata.mmd");
