...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_groups_align"];

#[test]
fn architecture_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/architecture/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/architecture")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Architecture(_)));
...
#[test]
fn architecture_animation_collapses_to_static_frame() {
    let source = include_str!("../../../tests/snapshots/architecture/input/02_groups_align.mmd");
