...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_labels_text_styles"];

#[test]
fn venn_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/venn/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/venn")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Venn(_)));
...
#[test]
fn venn_animation_collapses_to_static_frame() {
    let source = include_str!("../../../tests/snapshots/venn/input/02_labels_text_styles.mmd");
