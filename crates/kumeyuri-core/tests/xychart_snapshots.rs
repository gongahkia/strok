...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_horizontal_directive"];

#[test]
fn xychart_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/xychart/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/xychart")
        ...
        assert!(matches!(diagram.kind, DiagramKind::XyChart(_)));
