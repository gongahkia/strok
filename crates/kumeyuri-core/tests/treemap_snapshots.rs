...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_hierarchy_styles"];

#[test]
fn treemap_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/treemap/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/treemap")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Treemap(_)));
...
#[test]
fn treemap_animation_collapses_to_static_frame() {
    let source = include_str!("../../../tests/snapshots/treemap/input/02_hierarchy_styles.mmd");
