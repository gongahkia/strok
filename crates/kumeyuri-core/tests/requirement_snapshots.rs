...
const SNAPSHOT_NAMES: [&str; 3] = ["01_basic", "02_direction_types", "03_style_comment"];

#[test]
fn requirement_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/requirement/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/requirement")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Requirement(_)));
