...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_classes_comments"];

#[test]
fn quadrant_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/quadrant/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/quadrant")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Quadrant(_)));
