...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_expression"];

#[test]
fn railroad_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/railroad/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/railroad")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Railroad(_)));
...
#[test]
fn railroad_animation_collapses_to_static_frame() {
    let source = include_str!("../../../tests/snapshots/railroad/input/02_expression.mmd");
