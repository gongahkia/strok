...
const SNAPSHOT_NAMES: [&str; 2] = ["01_basic", "02_fragments_create"];

#[test]
fn zenuml_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/zenuml/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/zenuml")
        ...
        assert!(matches!(diagram.kind, DiagramKind::ZenUml(_)));
