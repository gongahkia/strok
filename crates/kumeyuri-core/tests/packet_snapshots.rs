...
const SNAPSHOT_NAMES: [&str; 2] = ["01_tcp", "02_bits_shorthand"];

#[test]
fn packet_static_snapshots_match() {
    ...
    for name in SNAPSHOT_NAMES {
        let source_path = root
            .join("tests/snapshots/packet/input")
            .join(format!("{name}.mmd"));
        let expected_path = root
            .join("tests/golden/kumeyuri-static/packet")
        ...
        assert!(matches!(diagram.kind, DiagramKind::Packet(_)));
...
#[test]
fn packet_animation_collapses_to_static_frame() {
    let source = include_str!("../../../tests/snapshots/packet/input/02_bits_shorthand.mmd");
