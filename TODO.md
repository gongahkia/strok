...
- [~] **O3. Asciinema re-stylise.** DoD: `--input recording.cast` replays through the render graph at original pacing; minimal VTE-lite handles SGR + cursor + scroll + clear; structure mode on a `htop` cast yields a stylised but readable version. Reference: PHASE_O §Asciinema.
  - `asciinema_in_tests` cover v2 header and event-line parsing; `asciinema_vte_tests` cover VTE-lite SGR, cursor, scroll, and clear handling; `asciinema_source_tests` cover output-event frames with original event timestamps. Open: player routing and render graph restyling.
