...
- [x] **P8. Benchmarks final sweep.** DoD: BENCHMARKS.md rows for every mode (luminance / structure-HoG / structure-SDF / octant / sextant / halfblock / braille / blocks / hatch / stipple / painterly / flow / pixel-Kitty) × 720p/1080p × CPU/GPU, with reproducible commands and ±10% repeatability. Reference: PHASE_P §Bench.
  - BENCHMARKS.md now records the 2026-06-26 final sweep across 52 rows: 13 modes × 720p/1080p × CPU/GPU. The primary sweep plus pinned-worker rerun for nine unstable rows keeps published `render_us` spread at or below 9.22%; both reproduction commands are documented.
