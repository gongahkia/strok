...
- [~] **L4. OBJ scene loader + tiny rasteriser.** DoD: `--input scene.obj` rotates Suzanne at 320×120 cells at ≥30 fps producing albedo + depth + normal G-buffers. Reference: PHASE_L §SceneInput.
  - OBJ parser plus CPU albedo/depth/normal G-buffer rasterizer landed; bundled scene alias and turntable/orbit/fly camera rendering are tested; `.obj`/`contourtty:scene:*` playback routes generated scene frames through `renderFrame`. BENCHMARKS.md records a bundled cube PTY run at 320×120 cells above 30 fps. Open: bundled Suzanne asset blocked by no verified license-clean source.
...
- [~] **M4. Temporal supersampling.** DoD: `--temporal-supersample N` decodes at N× source fps when source <30 fps; emits at source fps; cleaner orientations recorded on cube clip at the cost of ~1.5× decode CPU. Reference: PHASE_M §Supersample.
  - `--temporal-supersample N` now parses, is documented in help/man/README, and blends adjacent decoded luminance frames for analysis without changing emitted frame count; tests cover parse bounds and stats. BENCHMARKS.md records N=1 vs N=2 cost/output rows. Open: true N× decode/source-fps gate if the original libavfilter-style DoD is kept.
- [~] **M5. OKLab Δ diff threshold.** DoD: `--diff-oklab-eps` filters sub-perceptual SGR re-emits; bytes-per-frame on the pan clip drops 20–40% vs v0.5 with visuals unchanged within tolerance. Reference: PHASE_M §EmitDeltaOklab.
  - `--diff-oklab-eps` suppresses subthreshold truecolor SGR re-emits against the last emitted state. BENCHMARKS.md records a synthetic subtle-pan truecolor row with 67.0% bytes/frame reduction at eps 0.01 vs eps 0. Open: exact v0.5 reference-clip comparison/target range.
...
- [~] **O4. Bundled scenes + camera presets.** DoD: `contourtty --input contourtty:scene:suzanne --style cell-shade` works with no extra files; `--scene-camera turntable|orbit|fly` selectable. Reference: PHASE_O §SceneSource.
  - `contourtty:scene:cube` resolves to an installed OBJ asset; `--scene-camera turntable|orbit|fly` parses and renders distinct camera paths; scene playback generates rotating frames for `.obj` and bundled scene inputs; `--style cell-shade` parses and drives the OKLab posterize pass. Open: bundled Suzanne.
...
- [~] **P7. Finish packaging.** DoD: `brew install contourtty` on fresh macOS; `apt install ./contourtty_*.deb` on Ubuntu; GitHub Releases binaries run on clean machines with documented FFmpeg dependency; static-FFmpeg link offered for self-contained builds. Reference: PHASE_P §Packaging.
  - Package preflight checks for CMake/CPack/pkg-config and required FFmpeg/FreeType packages landed; local macOS TGZ package build and packaged binary smoke passed. README now states packages still link system FFmpeg/FreeType/zlib. Open: hosted releases, Linux `.deb` install proof, versioned Homebrew bottle, and static-FFmpeg release option.
