...
- [ ] **G2. Optional GPU compute path.** DoD: a compute-shader (Vulkan/OpenGL/compute via a chosen API) implementation of the analysis pass behind `--gpu`; falls back to CPU if unavailable; significant fps gain at high cell counts recorded. Reference: PHASE_G §GPU.
  ...
  - 2026-06-19: extended the Metal path to per-cell gradient aggregation and shape-glyph selection with kernel-level and renderer-level CPU parity tests. 160x45 structure export on 4x 1280x720 frames matched CPU byte-for-byte and improved wall time from `0.76s` to `0.36s`; a 240x80 / 1920x1080 smoke did not improve wall/render time, and DoG remains CPU-side, so G2 stays unchecked.
  - 2026-06-19: Metal structure path now also emits per-cell average RGB, eliminating CPU `averageRegion` for `--gpu` structure frames. 240x80 structure export on 4x 1920x1080 frames matched CPU byte-for-byte and improved wall time from `1.15s` to `0.75s`, but render time stayed flat and DoG remains CPU-side, so G2 stays unchecked.
  - 2026-06-19: fixed the GPU success path so it no longer computes an unused CPU Sobel field. 240x80 structure export on 4x 1920x1080 frames matched CPU byte-for-byte and improved from `1.10s` CPU to `0.40s` GPU wall time (`493169us` to `217074us` render time). DoG remains CPU-side, so G2 stays unchecked.
