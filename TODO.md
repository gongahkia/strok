...
- [~] **O4. Bundled scenes + camera presets.** DoD: `contourtty --input contourtty:scene:suzanne --style cell-shade` works with no extra files; `--scene-camera turntable|orbit|fly` selectable. Reference: PHASE_O §SceneSource.
  - `contourtty:scene:cube` resolves to an installed OBJ asset; `--scene-camera turntable|orbit|fly` parses; scene playback generates rotating frames for `.obj` and bundled scene inputs. Open: bundled Suzanne and `cell-shade` style.
