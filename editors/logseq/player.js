...
async function mount(element) {
  try {
    ...
    const cast = validateCast(await response.json());
    renderPlayer(element, cast);
  } catch (error) {
    element.classList.add("kumeyuri-logseq-error");
...
function renderPlayer(element, cast) {
  ...
  pre.className = "kumeyuri-logseq-frame";
  pre.setAttribute("aria-live", "polite");
  const toolbar = document.createElement("div");
  toolbar.className = "kumeyuri-logseq-controls";
