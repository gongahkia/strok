...
bool sameCell(const Cell& lhs, const Cell& rhs) noexcept {
  return lhs.glyph == rhs.glyph && sameColor(lhs.fg, rhs.fg) && sameColor(lhs.bg, rhs.bg);
}

}  // namespace

EmissionResult DiffEmitter::emit(const CellBuffer& current) {
  ...
  for (int row = 0; row < current.rows(); ++row) {
    for (int col = 0; col < current.cols(); ++col) {
      ...
      if (!full_repaint && sameCell(cell, previous_.at(col, row))) {
        continue;
      }
