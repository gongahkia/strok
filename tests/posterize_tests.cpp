...
bool throwsInvalidLevels() {
  try {
    (void)contourtty::posterizeOklab(contourtty::Rgb{.r = 128, .g = 128, .b = 128}, 1);
...
int main() {
  const contourtty::Rgb gray{.r = 128, .g = 128, .b = 128};
  const contourtty::Oklab lab = contourtty::rgbToOklab(gray);
  const contourtty::Rgb round_trip = contourtty::oklabToRgb(lab);
  ...
  const contourtty::Rgb posterized = contourtty::posterizeOklab(gray, 4);
  const double l = contourtty::rgbToOklab(posterized).l;
  ...
  const contourtty::Frame output = contourtty::posterizeFrameOklab(frame, 4);
