...
int main() {
  {
    ...
    const strok::SixelImage image = strok::quantizeSixelOklab(rgb, 2, 6);
    ...
    const std::string encoded = strok::encodeSixelRgb24(rgb, 2, 6);
...

  {
    bool threw = false;
    try {
      (void)strok::encodeSixelRgb24(std::vector<uint8_t>{255, 0, 0}, 2, 1);
