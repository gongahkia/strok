...
int main() {
  const std::filesystem::path fixture = std::filesystem::path(STROK_SOURCE_DIR) / "docs/v0.5-structure-demo.gif";
  strok::VideoDecoder decoder(fixture);
  expect(decoder.averageFps().has_value() && *decoder.averageFps() > 0.0, "decoder reports positive average fps");
  const std::optional<strok::Frame> first = decoder.nextFrame();
  expect(first.has_value(), "first frame exists");
  expect(first->pts_us == 0, "first frame starts at zero");

  decoder.seekToUs(300000);
  const std::optional<strok::Frame> sought = decoder.nextFrame();
  ...
  const std::optional<strok::Frame> next = decoder.nextFrame();
