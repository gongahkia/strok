...
int main() {
  ...
  options.width = 200;
  options.height = 100;
  const strok::RenderSize explicit_size = strok::fitRenderSize(frame, options, terminal);
  expect(explicit_size.cols == 200 && explicit_size.rows == 50, "explicit bounds without fit");

  options.fit = true;
  const strok::RenderSize fit_size = strok::fitRenderSize(frame, options, terminal);
