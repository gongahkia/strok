...
contourtty::Frame grayFrame(int width, int height) {
...
int main() {
  const contourtty::Frame frame = grayFrame(4, 4);
  const auto field = contourtty::makeLuminanceField(frame);
  expect(field.width == 4 && field.height == 4, "field dimensions");
  expect(field.values.size() == 16, "field stores every source pixel");

  const auto top_left = contourtty::sampleCellRegion(field, 2, 2, 0, 0);
  ...
  expectNear(top_left.at(0, 0), contourtty::relativeLuminance(contourtty::Rgb{.r = 0, .g = 0, .b = 0}), "top-left sample 0");
  expectNear(top_left.at(1, 0), contourtty::relativeLuminance(contourtty::Rgb{.r = 1, .g = 1, .b = 1}), "top-left sample 1");
  expectNear(top_left.at(0, 1), contourtty::relativeLuminance(contourtty::Rgb{.r = 4, .g = 4, .b = 4}), "top-left sample 4");
  expectNear(top_left.at(1, 1), contourtty::relativeLuminance(contourtty::Rgb{.r = 5, .g = 5, .b = 5}), "top-left sample 5");

  const auto bottom_right_uneven = contourtty::cellSourceRegion(5, 3, 2, 2, 1, 1);
